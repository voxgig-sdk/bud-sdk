...
boru-driven command-line client **and** interactive REPL for the Bud
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bud-cli)
make build
...
./bud-cli list account
./bud-cli load 1 account            # {id:1} shorthand
./bud-cli load '{id:1}' account       # explicit match map

# 5. Override the API base URL for a single call
BUD_BASE=https://api.example.com ./bud-cli list account
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bud-cli
   ```
...
```sh
./bud-cli load 1 account          # scalar shorthand for {id:1}
./bud-cli load '{id:1}' account     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bud>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bud-cli`. |
...
(`./bud-cli <expr>`) and interactively (the REPL), and expressions compose the
