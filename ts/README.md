...
The TypeScript SDK for the Bud API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Account()` — each with a small set of operations (`list`, `load`)
...
```ts
import { BudSDK } from '@voxgig-sdk/bud'
...
```ts
const client = new BudSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `BudSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `BudSDK.test(testopts?, sdkopts?)` | `BudSDK` | Create a test-mode client. |
...
| `client` | `client(): BudSDK` | Return the parent SDK client. |
