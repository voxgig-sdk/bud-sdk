...
function isRunnable(code) {
    return (/new\s+BudSDK\b/.test(code) ||
        /\bBudSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BudSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BudSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
