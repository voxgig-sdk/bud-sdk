    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BUD_TEST_ACCOUNT_ENTID JSON to run live")
        client = setup["client"]
...
def _account_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BUD_TEST_ACCOUNT_ENTID"))
    ...
    if env.get("BUD_TEST_LIVE") == "TRUE":
        ...
        client = BudSDK(helpers.to_map(merged_opts))

    _live = env.get("BUD_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BUD_TEST_EXPLAIN") == "TRUE",
        "live": _live,
