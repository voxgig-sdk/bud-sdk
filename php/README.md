...
The PHP SDK for the Bud API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Account()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/bud-sdk/releases](https://github.com/voxgig-sdk/bud-sdk/releases)
...
try {
    ...
    foreach ($accounts as $record) {
        ...
        echo $item["id"] . " " . $item["account_type"] . "\n";
    }
} catch (\Throwable $err) {
...
```php
$client = BudSDK::test($testopts, $sdkopts);
```
...
The main class (`bud_sdk.php`) exports the SDK class
