...
class BudMakeError
{
    public static function call(?BudContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BudContext([], null);
        }
        $op = $ctx->op ?? new BudOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BudResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BudError) ? $err->msg : (string)$err;
