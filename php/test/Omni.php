...
//    only - a typed BudContext there would make every ctx
...
final class BudOmni
{
    ...
    public static function err_from_map($m): ?BudError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BudError(is_string($code) ? $code : '', $msg);
    }
