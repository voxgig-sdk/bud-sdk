...
class FtCtrl extends BudControl
{
...
class FtClient
{
    ...
    private BudUtility $utility;

    public function __construct(BudUtility $utility, array $options)
    {
    ...
    public function get_utility(): BudUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BudContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BudUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BudContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(BudSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BudError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BudResponse($fetched);
        }
    ...
    private function fail(BudContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BudContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
