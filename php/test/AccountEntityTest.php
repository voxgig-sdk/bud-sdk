...
class AccountEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = BudSDK::test($seed, null);
        $seen = iterator_to_array($base->Account(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = BudConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = BudSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BUD_TEST_ACCOUNT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function account_basic_setup($extra)
{
    ...
    $client = BudSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("BUD_TEST_ACCOUNT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BUD_TEST_ACCOUNT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BUD_TEST_LIVE"] === "TRUE") {
        ...
        $client = new BudSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BUD_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
