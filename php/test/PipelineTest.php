...
require_once __DIR__ . '/../bud_sdk.php';
...
{
    private static function utility(): BudUtility
    {
        return new BudUtility();
    }
    ...
    private static function ctx(array $over = []): BudContext
    {
        ...
        $ctx->op = new BudOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BudError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BudOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BudMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BudOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BudMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BudOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BudMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BudSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BudMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BudResponse([]),
            'result' => new BudResult([]),
        ]);
        [, $err] = BudMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BudSpec(['step' => 's']),
            'response' => new BudResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BudResult([]),
        ]);
        [, $err] = BudMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BudSpec(['step' => 's']),
            'response' => new BudResponse(self::resp(200, ['v' => 1])),
            'result' => new BudResult([]),
        ]);
        [, $err] = BudMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BudSpec(['step' => 's']),
            'response' => new BudResponse(self::resp(200, ['v' => 2])),
            'result' => new BudResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BudMakeResponse::call($ctx);
    ...
    {
        $preset = new BudResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BudSpec([]),
            'response' => new BudResponse([]),
            'result' => new BudResult([]),
        ...
        [$got, $err] = BudMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BudResult([])]);
        [, $err] = BudMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BudSpec([]), 'result' => null]);
        [, $err] = BudMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BudSpec(['step' => 's']),
            'result' => new BudResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BudOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BudMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BudSpec(['step' => 's']),
            'result' => new BudResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BudOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BudMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BudResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BudSpec([]),
            'result' => new BudResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BudMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new BudError('boom', 'boom');
        $utility->fetcher = function (BudContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BudSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BudMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BudContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BudMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BudResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BudContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BudSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BudMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BudResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BudSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BudMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BudMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BudResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BudDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BudResult(['ok' => false])]);
        $this->expectException(BudError::class);
        BudDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BudResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BudMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BudBaseFeature();
        $b = new BudBaseFeature();
        BudFeatureAdd::call($ctx, $a);
        BudFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BudBaseFeature
    {
        $f = new BudBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): BudContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BudSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new BudResponse(['headers' => null]),
            'result' => new BudResult([]),
        ]);
        BudResultHeaders::call($ctx);
