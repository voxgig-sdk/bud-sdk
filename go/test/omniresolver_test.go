...
import (
	...
	omni "github.com/voxgig-sdk/bud-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BudError); is {
				out := map[string]any{
					"name":    "BudError",
...
func omniCtx(arg any, client *sdk.BudSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BudSDK); is {
			client = live
