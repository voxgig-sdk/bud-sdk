...
[MCP](https://modelcontextprotocol.io) server exposing the Bud SDK as
two agent tools — `bud_list` and `bud_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bud-mcp)
make build

# 2. Provide credentials via the environment
export BUD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bud \
  -- /absolute/path/to/bud-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bud-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bud_list: first page of records
{ "entity": "account" }
{ "entity": "account", "query": { } }

// bud_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bud-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bud \
     -- "$PWD"/dist/*/bud-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bud_list` and `bud_load` tools now appear
   in new sessions. Ask the agent to *"list account using bud"*
   and it calls `bud_list` with `{"entity":"account"}`.
...
```sh
export BUD_APIKEY=sk_live_xxx            # API key
export BUD_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bud_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bud_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BUD_APIKEY` | API key sent with every request. |
| `BUD_BASE` | Optional override of the API base URL. |
...
```sh
./bud-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bud_load","arguments":{"entity":"account","query":{"id":1}}}}'
```
