// Command Bud-mcp is an MCP server exposing the Bud
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Bud_list  — list records of an entity
//	Bud_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: BUD_APIKEY carries the API key and
	// BUD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BUD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BUD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBudSDK(opts)
